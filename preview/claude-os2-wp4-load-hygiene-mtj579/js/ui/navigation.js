...
function _warmRenderMultiPose(player){
  ...
  const _wrSpan = (name, fn) => {
    if(!window.perfMark){ fn(); return; }
    perfMark('goToRace:wr:'+name+':start');
    fn();
    perfMark('goToRace:wr:'+name+':end');
    perfMeasure('goToRace.warmRender.'+name,'goToRace:wr:'+name+':start','goToRace:wr:'+name+':end');
  };
  ...
  _wrSpan('poseA', () => _warmRenderOnePose(player, {
  ...
  _wrSpan('poseB', () => _warmRenderOnePose(player, {
    posX: px+_wrV1.x, posY: py+_wrV1.y, posZ: pz+_wrV1.z,
    lookX: px+_wrV2.x, lookY: py+_wrV2.y, lookZ: pz+_wrV2.z,
    fov: _wrBaseFov
  }));
  ...
  _wrSpan('poseC', () => _warmRenderOnePose(player, {
    posX: px+_wrV1.x, posY: py+_wrV1.y, posZ: pz+_wrV1.z,
    lookX: px+_wrV2.x, lookY: py+_wrV2.y, lookZ: pz+_wrV2.z,
    fov: 65
  }));
  ...
  _wrSpan('poseD', () => _warmRenderOnePose(player, {
...
async function goToRace(){
  ...
  try{
    ...
    if(typeof window._startSunArc === 'function'
      ...
      // WP4 rang 7 — dit blok (sync renderer.compile + shadow-render) zat
      // tussen goToRace.warmRender en goToRace.warmRaceTick in en was in de
      // harnas onattribueerbaar. Eigen span zodat de kost zichtbaar wordt.
      if(window.perfMark)perfMark('goToRace:sunArcPrewarm:start');
      try{
        window._startSunArc();
      ...
      if(window.perfMark){perfMark('goToRace:sunArcPrewarm:end');perfMeasure('goToRace.sunArcPrewarm','goToRace:sunArcPrewarm:start','goToRace:sunArcPrewarm:end');}
    }
