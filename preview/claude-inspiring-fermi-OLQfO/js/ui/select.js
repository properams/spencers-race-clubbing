...
function _startPreviewLoop(){
  ...
  // loadperf (H3): de live-preview spin is een continue rAF-loop op een
  // tweede WebGL-context. Mark start/stop zodat de eigenaar in de timeline
  // ziet of de spin het world-build/compile-venster overlapt (verklaart de
  // gevoelde traagheid bij het openen van de auto-kiezer).
  if(window.perfMark)perfMark('loadperf:carselect:spin-start');
  _previewActive=true;
...
function _stopPreviewLoop(){
  if(_previewActive && window.perfMark)perfMark('loadperf:carselect:spin-stop');
